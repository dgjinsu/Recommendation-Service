...
import fastcampus.directionsService.api.dto.MetaDto
...
class KakaoAddressSearchServiceRetryTest extends AbstractIntegrationContainerBaseTest {
    ...
    def "requestAddressSearch retry success"() {
        ...
        def documentDto = DocumentDto.builder()
                ...
                .build()
        def expectedResponse = new KakaoApiResponseDto(metaDto, Arrays.asList(documentDto)) //임의로 만든 응답 데이터
        def uri = mockWebServer.url("/").uri()

        when:
        mockWebServer.enqueue(new MockResponse().setResponseCode(504)) //첫번째 요청은 500번대 에러
        mockWebServer.enqueue(new MockResponse().setResponseCode(200) //두번째 요청에 대한 응답을 설정
        ...
        then:
        2 * kakaoUriBuilderService.buildUriByAddressSearch(inputAddress) >> uri //두번 호출된게 맞는지
